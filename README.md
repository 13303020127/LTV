...
#### 准备工作
1. 确保项目中包含以下 Cloudflare Pages 配置文件：
   - `_headers`: 定义 HTTP 响应头和安全配置
   - `_redirects`: 定义 URL 重定向规则
   - `wrangler.toml`: Wrangler 配置文件

#### 部署步骤
1. **创建 Cloudflare Pages 项目**
   - 在 [Cloudflare Dashboard](https://dash.cloudflare.com/) 中，选择 **Pages**
   - 点击 **连接到 Git**
...
3. **配置环境变量**
   - 在 **环境变量** 部分，添加以下环境变量：
     - 必要环境变量（根据您的存储需求选择）：
       - `UPSTASH_REDIS_REST_URL` 和 `UPSTASH_REDIS_REST_TOKEN`（推荐使用Upstash Redis）
       - 或 `REDIS_URL`（使用其他Redis服务）
     - 可选环境变量：
       - `ADMIN_PASSWORD`: 管理员密码
       - `NEXT_PUBLIC_STORAGE_TYPE`: 存储类型（'localstorage' | 'redis' | 'upstash' | 'kvrocks'）
       - `NEXT_PUBLIC_ENABLE_REGISTRATION`: 是否启用用户注册

4. **部署完成后**
   - 部署成功后，您可以通过 Cloudflare Pages 提供的 URL 访问应用
   - 首次访问需要配置播放源和直播源

### 其他部署方式

本项目也支持 Docker 或其他基于 Docker 的平台部署。
